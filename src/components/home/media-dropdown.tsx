...
import { sendMediaMessage } from "@/lib/supabase/messages";
...
const MediaDropdown = () => {
  ...
  const handleSendImage = async () => {
    ...
    try {
      ...
      const ok = await sendMediaMessage(selectedConversation._id, selectedImage, "image");
  ...
  const handleSendVideo = async () => {
    setIsLoading(true);
    try {
      if (!selectedConversation || !selectedVideo) return;
      const ok = await sendMediaMessage(selectedConversation._id, selectedVideo, "video");
      if (!ok) throw new Error("Failed to send video");
