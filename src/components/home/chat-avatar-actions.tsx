...
import { ensureDirectConversation, kickUserFromConversation } from "@/lib/supabase/conversations";
...
const ChatAvatarActions = ({ message, me }: ChatAvatarActionsProps) => {
  ...
  const handleKickUser = async (e: React.MouseEvent) => {
    ...
    try {
      await kickUserFromConversation(selectedConversation._id as string, message.sender._id as string);
  ...
  const handleCreateConversation = async () => {
    if (fromAI) return;
    try {
      const conversationId = await ensureDirectConversation(message.sender._id as string);
